...
class ParserState
{
    ...
    ParserState(int order)
    {
        ...
        yearBeforeDay =
            (order & YEAR_BEFORE_DAY) == YEAR_BEFORE_DAY;
        monthBeforeDay =
            (order & MONTH_BEFORE_DAY) == MONTH_BEFORE_DAY;
...
public class CalendarParser
{
    ...
    public static final int DD_MM_YY =
        YEAR_AFTER_MONTH | YEAR_AFTER_DAY | MONTH_AFTER_DAY;
    /** month/day/year order. */
    public static final int MM_DD_YY =
        YEAR_AFTER_MONTH | YEAR_AFTER_DAY | MONTH_BEFORE_DAY;
    /** month/year/day order. */
    public static final int MM_YY_DD =
        YEAR_AFTER_MONTH | YEAR_BEFORE_DAY | MONTH_BEFORE_DAY;
    /** day/year/month order. */
    public static final int DD_YY_MM =
        YEAR_BEFORE_MONTH | YEAR_AFTER_DAY | MONTH_AFTER_DAY;
    /** year/day/month order. */
    public static final int YY_DD_MM =
        YEAR_BEFORE_MONTH | YEAR_BEFORE_DAY | MONTH_AFTER_DAY;
    /** year/month/day order. */
    public static final int YY_MM_DD =
        YEAR_BEFORE_MONTH | YEAR_BEFORE_DAY | MONTH_BEFORE_DAY;
    ...
    private static final int getOrdinalNumber(String str)
    {
        ...
        if (len >= 3) {
            ...
            if (suffix.equalsIgnoreCase("st") ||
                suffix.equalsIgnoreCase("nd") ||
     ...
     *
     * @param dateStr date string
     * @param order order in which pieces of numeric strings
     *              are assigned (should be one of <tt>YY_MM_DD</tt>,
     *              <tt>MM_DD_YY</tt>, etc.)
     *
     * @return parsed date
     *
     ...
     *              are assigned (should be one of <tt>YY_MM_DD</tt>,
     *              <tt>MM_DD_YY</tt>, etc.)
     * @param ignoreChanges if <tt>true</tt>, ignore date changes such as
     *                      <tt>Feb 31</tt> being changed to <tt>Mar 3</tt>.
     *
     * @return parsed date
     *
     * @throws CalendarParserException if there was a problem
        ...
        throws CalendarParserException
    {
        ...
        if (isWeekdayName(token)) {
            if (DEBUG) {
                System.err.println("IGNORE \"" + token +
                                   "\" (weekday)");
            }
            return;
        }
        ...
        if (tmpMon != UNSET) {
            ...
            if (DEBUG) {
                System.err.println("*** Too many numbers in \"" +
                                   dateStr + "\"");
            }
        ...
        throws CalendarParserException
    {
        ...
        if (val > 31) {
            ...
            if (state.getYear() > 31) {
                if (DEBUG) {
                    System.err.println("*** Ambiguous year " +
                                       state.getYear() + " vs. " + val);
                }
                String errMsg =
                    "Couldn't decide on year number in date \"" +
                    dateStr + "\"";
        ...
        if (val > 12) {
            ...
            if (DEBUG) {
                System.err.println("*** Unassignable year/day number " +
                                   val);
            }
            throw new CalendarParserException("Bad number " + val +
        ...
        if (state.isYearSet()) {
            if (state.isMonthSet() || (!state.isDateSet() &&
                                       !state.isMonthBeforeDay()))
            {
                state.setDate(val);
                if (DEBUG) {
                    System.err.println("DAY=" + state.getDate() + " (" +
        ...
        throws CalendarParserException
    {
        ...
        while (prevEnd < dateStr.length()) {
            ...
            if (DEBUG) {
                System.err.println("YEAR " +
                                   (state.isYearSet() ?
                                    Integer.toString(state.getYear()) :
                                    "UNSET") +
                                   ", MONTH " +
                                   (state.isMonthSet() ?
                                    Integer.toString(state.getMonth()) :
                                    "UNSET") +
                                   ", DAY " +
                                   (state.isDateSet() ?
                                    Integer.toString(state.getDate()) :
                                    "UNSET") +
                                   ", TOKEN=\"" + token + "\"");
            }
        ...
        if (!state.isDateSet()) {
            ...
            throw new CalendarParserException("Year missing from \"" +
                                              dateStr + "\"");
        }
        ...
        if (DEBUG) {
            System.err.println("Y" + state.getYear() +
                               " M" + state.getMonth() +
                               " D" + state.getDate() +
                               " H" + state.getHour() +
                               " M" + state.getMinute() +
                               " S" + state.getSecond() +
                               " L" + state.getMillisecond() +
                               " => " + toString(cal));
        }
        ...
        for (int i = 0; i < tList.length; i++) {
            ...
            if (DEBUG) {
                System.err.println("HOUR " +
                                   (state.isHourSet() ?
                                    Integer.toString(state.getHour()) :
                                    "UNSET") +
                                   ", MINUTE " +
                                   (state.isMinuteSet() ?
                                    Integer.toString(state.getMinute()) :
                                    "UNSET") +
                                   ", SECOND " +
                                   (state.isSecondSet() ?
                                    Integer.toString(state.getSecond()) :
                                    "UNSET") +
                                   ", MILLISECOND " +
                                   (state.isMillisecondSet() ?
                                    Integer.toString(state.getMillisecond()) :
                                    "UNSET") +
                                   ", TOKEN=\"" + token + "\"");
            }
